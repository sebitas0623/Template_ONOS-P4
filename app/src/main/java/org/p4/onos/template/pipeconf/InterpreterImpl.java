...
import org.p4.onos.template.AppConstants;
...
public class InterpreterImpl extends AbstractHandlerBehaviour
    ...
    // From P4Info.
    //------------------------------------ ID de las acciones del pipeline p4 ----------------------------------------
    private static final PiActionId ACT_ID_NOP = PiActionId.of("NoAction");
    private static final PiActionId ACT_ID_SEND_TO_CPU = PiActionId.of("c_ingress.send_to_cpu");
    private static final PiActionId ACT_ID_SET_EGRESS_PORT = PiActionId.of("c_ingress.set_out_port");

    //--------------------------------------- Mapeo de los campos de Match -------------------------------------------
    ...
    //--------------------------------------- Mapeo del ID de la tabla t_l2_fwd ---------------------------------------
    private static final Map<Integer, String> TABLE_MAP =
            new ImmutableMap.Builder<Integer, String>()
                    .put(0,"c_ingress.t_l2_fwd")
                    .build();

    private static final PiTableId TABLE_L2_FWD_ID = PiTableId.of("c_ingress.t_l2_fwd");
    //-----------------------------------------------------------------------------------------------------------------
    ...
    public PiAction mapTreatment(TrafficTreatment treatment, PiTableId piTableId)
        ...
        /*if (piTableId != TABLE_L2_FWD_ID){
            log.info("piTableId = {} , TABLE_L2_FWD_ID = {}", piTableId, TABLE_L2_FWD_ID);
            throw new PiInterpreterException("Solo se puede mapear TREATMENT para la tabla 't_l2_fwd'");
        }*/

        if (treatment.allInstructions().size() == 0){
            return PiAction.builder().withId(ACT_ID_NOP).build(); // 0 instrucciones, accion 'NoAction'
        }else if (treatment.allInstructions().size() > 1){
            throw new PiInterpreterException("Treatment tiene multiples instrucciones");
        }

        Instruction instruction = treatment.allInstructions().get(0);

        if (instruction.type() != OUTPUT) {
            throw new PiInterpreterException(format("Instruccion de tipo '%s' no es soportado", instruction.type()));
        }
        ...
        if (!port.isLogical()) {
            ...
            throw new PiInterpreterException(format(
                    "El puerto logico de salida '%s' no es soportado", port));
        }
    }

    @Override
    public Optional<PiTableId> mapFlowRuleTableId(int flowRuleTableId) {
        if (TABLE_MAP.containsKey(flowRuleTableId)){
            return Optional.of(PiTableId.of(TABLE_MAP.get(flowRuleTableId)));
        } else {
            return Optional.empty();
        }
    }
